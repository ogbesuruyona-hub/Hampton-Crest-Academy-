...
`server.py` has grown to ~1,800 lines. Goal: split into single-responsibility
...
helpers, the seeders) and calls each factory at startup time.
...
### Suggested split order (safe → invasive)
...
5. **content** — research / education / reports / companies / books / bookmarks (heaviest, do last)
...
- Seed functions (`seed_admin`, `seed_test_member`)

When all routes have moved out, this file can shrink to ~150 lines:
configuration + app construction + CORS + startup hooks.

## Why not refactor it all now?

The current monolithic `server.py` works, is fully tested (104/104 in the last
suite), and ships value. Refactoring it as a single change is high-risk for
zero user-visible gain. The pattern above lets us extract one router per
session, validate with tests, and arrive at the target structure incrementally.
